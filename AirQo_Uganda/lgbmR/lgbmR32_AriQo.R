...
library(caret)
set.seed(123)
myFolds <- createFolds(y = dataTrain$target, k = 10, list = TRUE)
index <-list()
...
for (i in 1:k) {
  ...
  predictions <- predict(model,
                         data.matrix(dataTest %>% select(-ID)),
...
write.csv(lgbmR32_AirQo, file = "Submission/lgbmR32_AirQo.csv", row.names = FALSE)

# Submission
predicciones2 <- dataPred$Mod9
dataTest %>% 
  select(ID) %>% 
  mutate(target = predicciones2) ->
  lgbmR33_AirQo
write.csv(lgbmR33_AirQo, file = "Submission/lgbmR33_AirQo.csv", row.names = FALSE)

# Submission
predicciones3 <- apply(dataPred[, c(2, 7, 9)], 1, mean)
dataTest %>% 
  select(ID) %>% 
  mutate(target = predicciones3) ->
  lgbmR34_AirQo
write.csv(lgbmR34_AirQo, file = "Submission/lgbmR34_AirQo.csv", row.names = FALSE)
